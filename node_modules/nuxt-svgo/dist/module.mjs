...
function svgLoader(options) {
  ...
  const autoImportPaths = autoImportPath ? (Array.isArray(autoImportPath) ? autoImportPath : [autoImportPath]).map(
    (p) => p.replaceAll(/^\.*(?=[/\\])/g, "")
  ) : [];
    ...
    async load(id) {
      ...
      if (explicitImportsOnly) {
        ...
        if (!isExplicitlyQueried) {
          const isInAutoImportPath = autoImportPaths.length > 0 && autoImportPaths.some((p) => path.includes(p));
...
const nuxtSvgo = defineNuxtModule({
  ...
  async setup(options, nuxt) {
    ...
    if (options.autoImportPath) {
      ...
      const autoPaths = Array.isArray(options.autoImportPath) ? options.autoImportPath : [options.autoImportPath];
